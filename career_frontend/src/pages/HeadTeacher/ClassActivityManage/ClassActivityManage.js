...
import AddActivityNotice from './AddActivityNotice';
...
class ClassActivityManage extends Component {
  ...
  handleOk = () => {
    ...
    form.validateFields((err, fieldsValue) => {
      ...
      ClassJson.forEach(item => {
        ...
        data.ClassJson.push(item);
      })

      data.ClassJson = JSON.stringify(data.ClassJson);
      if(Image) data.Image = Image.fileList.map(item => item.response.data.ImgUrl);
      console.log(data)
      dispatch({
        type: 'classActivityManage/reqCommon',
  ...
  handleNoticeOk=()=>{
    ...
    form.validateFields((err, fieldsValue)=>{
        ...
        data.Notice=fieldsValue.Notice,
        data.Class=JSON.stringify(fieldsValue.Class)
      //  for(k in fieldsValue.Class){
      //    console.log(k)
      //   data.Class.push(fieldsValue.Class[k])
      //  }
  ...
  render() {
    ...
    const { visible, editVisible ,NoticeVisibale} = this.state;
    // console.log(classActivityList)
    return (
      <PageHeaderWrapper title="班级活动管理">
        ...
        <div style={{ background: '#fff', padding: '20px' }}>
          <Form layout="inline">
            ...
            <Form.Item>
              <Button type="primary" onClick={this.showNotice}>发布活动通知</Button>
            </Form.Item>
            <Form.Item>
