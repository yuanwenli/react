...
@connect(({ classActivityManage,studentActivity, loading}) => ({
  classMateList: studentActivity.classMateList,
...
class ClassActivity extends Component {
  ...
  render() {
    const { classMateList } = this.props;
    console.log(classMateList)
    return (
      classMateList && classMateList.list ? <List
        grid={{
          gutter: 16, xs: 1, sm: 2, md: 4, lg: 4, xl: 6, xxl: 3
        }}
        pagination={{total: classMateList.pages*10, onChange: this.handlePageChange}}
        dataSource={classMateList.list}
        renderItem={(item) => (
          <List.Item>
